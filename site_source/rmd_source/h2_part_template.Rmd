...
`r if(dat_sub$source=="finngen"){
  paste0("This phenotype is defined based on ICD10 codes to match an endpoint defined by [FinnGen](https://www.finngen.fi/en).")
}else if(isICD){
...
Download the full LDSR results for this phenotype, including SNP heritability and partitioning results, from the [link on the front page](index.html).
