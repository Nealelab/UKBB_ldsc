...
Explore results of `r ifelse(isUnivar, "univariate", "partitioned")` LD score regression for UK Biobank phenotypes. You can read more about this analysis [on the annoucement page](`http://`).
...
* **N**: The sample size in the UKBB analysis set (more info on the GWAS analysis [here]())
...
* **Heritability** ($h^2$): the SNP heritability estimate from `r ifelse(isUnivar, "univariate", "partitioned")` LD score regression. Value is given on the liability scale for binary phenotypes, assuming that the population prevalence matches the prevalence in the UK Biobank analysis set. P-value ($h^2$ p) corresponds to the test that the intercept is greater than 1.
