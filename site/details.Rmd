...
* A full technical discussion on the limitations of this analysis and recommendations for intepreting the results can be found on the [Neale Lab blog](`http://www.nealelab.is`).
...
* We’ve run both univariate and partitioned heritability analyses. For reasons that will be discussed in a later post we’re currently presenting the partitioned heritability LDSR results as the primary analysis in the summary browser (link), but the univariate results are also available [here](`https://nealelab.github.io/UKBB_ldsc/h2_univar_browser.html`).
